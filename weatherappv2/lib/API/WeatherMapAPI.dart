...
class WeatherMapAPI {
  ...
  static Future<List<Weather>> getForecast() async {
    List<Weather> weatherList = [];
    var url = Uri.parse("http://api.openweathermap.org/data/2.5/forecast?lat=$parisLat&lon=$parisLon&appid=$weatherMapApi&lang=fr&units=metric");
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0;i < l.length;i++) {
        DateTime dateTime = DateFormat("yyyy-MM-dd HH:mm:ss").parse(l[i]['dt_txt']);
        if (dateTime.hour == 12) {
          weatherList.add(Weather(
            ...
            weatherIconId: l[i]['weather'][0]['icon'],
          ));
        }
      }
      return weatherList;
